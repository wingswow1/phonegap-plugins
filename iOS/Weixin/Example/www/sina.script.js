...
function sendTextContent() {
    ...
    weixin.textContent(onSuccess,onError,"send","hello world");
}
...
function sendImageContent(){
    ...
    onCameraSuccess:function(imageURI){
        ...
        weixin.imageContent(onSuccess,onError,"send",imageURI,{
                                     title:"kris",
                                     description:"picture",
                                     });
    },
...
function sendMusicContent(){
    weixin.musicContent(onSuccess,onError,"send",
...
function sendVideoContent(){
    weixin.videoContent(onSuccess,onError,"send",
...
function sendAPPContent(){
    weixin.APPContent(onSuccess,onError,"send",
                           {
                           title:"App消息",
...
function getTextContent() {
    weixin.textContent(onSuccess,onError,"get","Sina App Engine");
    set2SendView();
}
...
function getImageContent(){
    ...
    onCameraSuccess:function(imageURI){
        console.log('imageURI:'+imageURI);
        weixin.imageContent(onSuccess,onError,"get",imageURI,{
                                    title:"kris",
                                    description:"picture",
                                    });
        set2SendView();
...
function getMusicContent(){
    ...
    onCameraSuccess:function(imageURI){
        console.log('imageURI:'+imageURI);
        weixin.musicContent(onSuccess,onError,"get",
                                    "http://pluginlist.sinaapp.com/client/music/Sunshine.mp3",
                                    {
                                    title:"Sunshine",
                                    description:"Happy Music",
...
function getVideoContent(){
    weixin.videoContent(onSuccess,onError,"get",
                             "http://www.tudou.com/listplay/0nYp1obVv60/mA_xdJq7lSo.html",
                             {
                             title:"video",
...
function getWebpageContent(){
    weixin.webpageContent(onSuccess,onError,"get",
                                  "http://sae.sina.com.cn/?m=devcenter&catId=235",
                                  {
                                  title:"新浪移动云平台介绍",
                                  description:"新浪移动云是在SAE基础上的子平台，专注于为移动设备同时提供云+端的能力。\n为方便开发者使用，移动云直接集成在SAE在线管理平台中。",
...
function getAPPContent(){
    weixin.APPContent(onSuccess,onError,"get",
                           {
                           title:"App消息",
                           description:"您的App消息到啦，快去看看吧",
                           thumbUrl:"http://pluginlist.sinaapp.com/client/images/icon.png",
...
function getWXAppInstallUrl(){
    weixin.getWXAppInstallUrl(function(resultUrl){
                                   var detail = document.getElementById("detail");
...
function isWeixinInstalled(){
    weixin.isWeixinInstalled(function(){
...
function setResponse(){
    weixin.setResponser("receiveResponse");
}
